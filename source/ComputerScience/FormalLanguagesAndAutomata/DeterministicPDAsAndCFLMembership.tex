...
This note explains deterministic pushdown automata and the CYK membership algorithm in a self-contained way.
...
Nondeterministic \sr{pushdown automaton}{pushdown automata} are the right level of generality for all context-free languages, but deterministic pushdown automata are still important.
...
This determinism condition has two parts:
\begin{enumerate}
...
A standard DPA example recognises the language
\[
...
The following accepting sequence illustrates the computation:
\[
(q_0,01c10,z_0)
\vdash (q_0,1c10,0z_0)
\vdash (q_0,c10,10z_0)
...
The previous DPA can be compared with an \sr{nondeterministic pushdown automaton}{NPDA} for
\[
L = \{ww^R \mid w \in \{0,1\}^*\}.
\]
...
Assuming \sn{final-state acceptance}, deterministic languages satisfy:
\begin{itemize}
...
\sr{nondeterministic pushdown automaton}{NPDAs} accepting by \sn{final-state acceptance} and by \sn{empty-stack acceptance} are equivalent in expressive power.
For DPAs, this is no longer true.
DPAs accepting by \sn{empty-stack acceptance} have very limited language-recognising power.
...
The language $\{0,00\}$ illustrates the argument.
...
The same reasoning gives the general statement.
...
This brute-force method is exponential in the length of $w$.
...
The following inference motivates the dynamic-programming recurrence.
Suppose
\[
w = w_1w_2\cdots w_n,
\]
...
Consider the \sr{Compilation?ContextFreeGrammar}{grammar}
\[
S \to AB,
\qquad
A \to BB \mid a,
...
The completed table can therefore be written as
\[
\begin{array}{ccccc}
\{S,B\} \\
\{A\} & \{S,B\} \\
...
This note develops three linked ideas:
\begin{itemize}
